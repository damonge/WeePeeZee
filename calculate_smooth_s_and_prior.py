        ...
        if os.path.isfile(covfn):
            ...
            for i in range(N_tracers):
                ...
                if upsample > 1:
                    ...
                    cov_CV_up_per_tracer = np.zeros((new_Nz_per_tracer,new_Nz_per_tracer))
                    intermediate_cov = np.zeros((Nz_per_tracer,new_Nz_per_tracer))
                    bor_cov_CV_up_per_tracer = np.zeros((new_Nz_per_tracer,new_Nz_per_tracer))
                    ...
                    for col in range(Nz_per_tracer*upsample):
                        fun = interpolate.interp1d(zs_data,intermediate_cov[:,col],fill_value="extrapolate")
